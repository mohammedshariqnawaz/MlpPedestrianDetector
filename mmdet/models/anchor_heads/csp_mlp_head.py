...
class CSPMLPHead(CSPHead):
    ...
    def __init__(self, *args, patch_dim=4, windowed_input=True, mlp_hidden_dim_factor=2, **kwargs):
    ...
    def _init_layers(self):
        self.mlp_with_feat_reduced = nn.Sequential(
            MixerBlock(self.patch_dim**2, self.in_channels, self.mlp_hidden_dim_factor),
            nn.Linear(self.in_channels, self.feat_channels)
        )

        self.pos_mlp = nn.Sequential(
            MixerBlock(self.patch_dim**2, self.feat_channels,self.mlp_hidden_dim_factor),
